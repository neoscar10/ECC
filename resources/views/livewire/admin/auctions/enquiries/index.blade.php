<div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Auction Enquiries</h4>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($enquiry->lot)
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div class="flex-grow-1">
                                                        {{ Str::limit($enquiry->lot->title, 20) }}
                                                    </div>
                                                </div>
                                            @else
                                                <span class="text-muted text-italic">Lot Not available</span>
                                            @endif
                                        </td>
                                        <td>
                                        ...
                                        <td class="status">
                                            @if($enquiry->status === 'new')
                                                <span class="text-uppercase mx-1 badge bg-info-subtle text-info">New</span>
                                            @elseif($enquiry->status === 'contacted')
                                                <span class="text-uppercase mx-1 badge bg-warning-subtle text-warning">Contacted</span>
                                            @elseif($enquiry->status === 'closed')
                                                <span class="text-uppercase mx-1 badge bg-success-subtle text-success">Closed</span>
                                            @else
    ...
    {{-- View Modal --}}
    <div wire:ignore.self wire:key="auction-enquiry-view-modal" class="modal fade" id="viewEnquiryModal" tabindex="-1" aria-labelledby="viewEnquiryModalLabel" aria-hidden="true">
                ...
                <div class="modal-body">
                    @if($selectedEnquiry)
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <h6 class="text-muted text-uppercase fw-semibold mb-3">Lot Information</h6>
                                @if($selectedEnquiry->lot)
                                    <div class="d-flex gap-3 mb-3">
                                        ...
                                        <div class="flex-grow-1">
                                            <h6 class="fs-14 mb-1">{{ $selectedEnquiry->lot->title }}</h6>
                                            <p class="text-muted mb-0">Lot #{{ $selectedEnquiry->lot->lot_no }}</p>
                                            <a href="{{ route('admin.auctions.lots.show', $selectedEnquiry->lot->id) }}" target="_blank" class="text-primary fs-12">View Lot</a>
                                        </div>
                                    </div>
                                @else
                                    <p class="text-danger">Lot has been deleted.</p>
                                @endif
