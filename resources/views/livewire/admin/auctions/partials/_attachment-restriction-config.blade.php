@php $row = $attachmentRows[$index] ?? []; @endphp
@if(($row['restriction_mode'] ?? 'inherit') === 'restricted')
    <div class="mt-2 p-2 bg-white border rounded">
        ...
        <select class="form-select form-select-sm mb-2" wire:model.live="attachmentRows.{{ $index }}.restriction_type">
        ...
        @if(isset($row['restriction_type']))
            @if($row['restriction_type'] === 'hierarchical')
                <select class="form-select form-select-sm" wire:model.live="attachmentRows.{{ $index }}.restricted_min_tier_id">
                    ...
                    @if(count($attachmentAllowedTiers) > 0)
                        @foreach($attachmentAllowedTiers as $tier)
                            <div class="form-check form-check-sm mb-1">
                                <input class="form-check-input" type="checkbox" value="{{ $tier['id'] }}" wire:model.live="attachmentRows.{{ $index }}.selected_tiers" id="att-rand-{{ $index }}-{{ $tier['id'] }}">
            ...
            @elseif($row['restriction_type'] === 'private')
                <select class="form-select form-select-sm" wire:model.live="attachmentRows.{{ $index }}.restricted_private_tier_id">
