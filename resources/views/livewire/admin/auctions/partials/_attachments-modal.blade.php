    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Manage Attachments (Auction Lot)</h5>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-3 border-end">
                        ...
                        <div class="mt-4 p-3 bg-light rounded fs-12 text-muted">
                            ...
                            <p class="mb-2">Attachments default to <strong>Inherit</strong> (same access as lot).</p>
                            <p class="mb-0">You can further restrict an attachment to a subset of the lot's allowed tiers.</p>
                        </div>
                    </div>
                        ...
                        <div class="tab-content text-muted mt-3 mt-lg-0" id="v-pills-tabContent">
                            ...
                            <div class="tab-pane fade show active" id="v-pills-quick" role="tabpanel" aria-labelledby="v-pills-quick-tab" wire:ignore.self>
                                ...
                                @foreach($attachmentRows as $index => $row)
                                    @if($row['type'] === 'line')
                                        @include('livewire.admin.auctions.partials._attachment-row-line', ['index' => $index, 'row' => $row])
                                    @endif
                                @endforeach
                            ...
                            <div class="tab-pane fade" id="v-pills-kv" role="tabpanel" aria-labelledby="v-pills-kv-tab" wire:ignore.self>
                                ...
                                @foreach($attachmentRows as $index => $row)
                                    @if($row['type'] === 'kv')
                                        @include('livewire.admin.auctions.partials._attachment-row-kv', ['index' => $index, 'row' => $row])
                                    @endif
                                @endforeach
                            </div>
                            ...
                            <div class="tab-pane fade" id="v-pills-rich" role="tabpanel" aria-labelledby="v-pills-rich-tab" wire:ignore.self>
                                ...
                                @foreach($attachmentRows as $index => $row)
                                    @if($row['type'] === 'rich')
                                        @include('livewire.admin.auctions.partials._attachment-row-rich', ['index' => $index, 'row' => $row])
                                    @endif
                                @endforeach
                            </div>
