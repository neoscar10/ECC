    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Contact Enquiries</h4>
                            ...
                            <tbody class="list form-check-all">
                                @forelse ($enquiries as $enquiry)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge bg-light text-body">{{ ucwords(str_replace('_', ' ', $enquiry->subject)) }}</span>
                                        </td>
                                        <td>
                                        ...
                                        <td class="status">
                                            @if($enquiry->status === 'new')
                                                <span class="badge bg-danger text-uppercase">New</span>
                                            @elseif($enquiry->status === 'contacted')
                                                <span class="badge bg-warning text-uppercase">Contacted</span>
                                            @elseif($enquiry->status === 'closed')
                                                <span class="badge bg-success text-uppercase">Closed</span>
                                            @else
                                                <span class="badge bg-info text-uppercase">{{ $enquiry->status }}</span>
                                            @endif
                                        </td>
                                        <td>
                                        ...
                                        <td colspan="7" class="text-center">
                                            <div class="noresult">
                                                <div class="text-center">
                                                    ...
                                                    <p class="text-muted mb-0">We did not find any enquiries for your search.</p>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                ...
                <div class="modal-body">
                    @if($selectedEnquiry)
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <h6 class="text-muted text-uppercase fw-semibold mb-3">Enquiry Information</h6>
                                <p class="mb-2"><span class="fw-medium">Subject:</span> {{ ucwords(str_replace('_', ' ', $selectedEnquiry->subject)) }}</p>
                                
                                <h6 class="text-muted text-uppercase fw-semibold mb-2 mt-4">Status Actions</h6>
                                <div class="d-flex gap-2">
                                    ...
                                    <button wire:click="updateStatus({{ $selectedEnquiry->id }}, 'viewed')" class="btn btn-sm {{ $selectedEnquiry->status === 'viewed' ? 'btn-primary' : 'btn-ghost-primary' }}">Viewed</button>
