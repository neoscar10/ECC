                                 ...
                                 <div class="d-flex align-items-center">
                                    <div class="avatar-sm flex-shrink-0 me-3">
                                        <div class="avatar-title bg-primary-subtle text-primary rounded-circle fs-20 uppercase">
                                            {{ substr($membership->user->name ?? 'U', 0, 1) }}
                                        </div>
                                    </div>
                                    <div class="flex-grow-1">
                                        <h5 class="fs-14 mb-1">{{ $membership->user->name ?? 'Unknown Member' }}</h5>
                                        <p class="text-muted mb-0 fs-12">{{ $membership->user->email ?? $membership->user->phone ?? '' }}</p>
                                    </div>
                                    <div class="flex-shrink-0 text-end">
                                        <span class="badge bg-secondary-subtle text-secondary fs-11">Current Tier</span>
                                        <h6 class="mb-0 mt-1 text-uppercase">{{ $membership->membershipTier->name ?? 'N/A' }}</h6>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('new_tier_id') is-invalid @enderror" id="new_tier_id" wire:model.live="new_tier_id">
                                ...
                                @foreach($tiers as $tier)
                                    <option value="{{ $tier->id }}" @disabled($currentTier && $tier->id == $currentTier->id)>
                                        {{ $tier->name }} 
