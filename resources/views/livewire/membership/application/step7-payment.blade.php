<div class="ecc-pay ecc-bg min-vh-100 position-relative overflow-hidden">
  ...
  <header class="ecc-topbar position-sticky top-0 z-3">
    <div class="container-fluid px-4 py-3 d-flex align-items-center justify-content-between">
      <button type="button" class="ecc-icon-btn" onclick="(history.length>1)?history.back():window.location.href='{{ route('membership.application.step6') }}'">
    ...
    <div class="container-fluid px-4 pb-3">
      <div class="d-flex align-items-center justify-content-between mx-auto ecc-max">
        <div class="ecc-step-kicker">STEP VII OF VIII</div>
      ...
      <section class="ecc-tier-sum mb-4">
        <div class="d-flex align-items-stretch justify-content-between gap-3">
          <div class="flex-grow-1">
            <div class="ecc-mini text-uppercase">Selected Tier</div>
        ...
        <div class="pt-3">
          <button type="submit"
            ...
            <span wire:loading.remove wire:target="submit" class="ecc-btn-load-wrapper">
                <span>Confirm & Pay {{ $amountFormattedPlain }}</span>
...
<style>
  :root{
    --ecc-primary:#D4AF37; --ecc-primary-dark:#AA8C2C;
    --ecc-bg:#020202; --ecc-surface:#0a0a0a; --ecc-border:rgba(212,175,55,.30);
  }
  .ecc-bg{ background:var(--ecc-bg); color:#fff; font-family:"Work Sans","Noto Sans",system-ui,sans-serif; }
  ...
  .ecc-bg-glow{ position:absolute; top:0; left:50%; transform:translateX(-50%); width:520px; height:320px; background:rgba(212,175,55,.10); filter:blur(110px); border-radius:9999px; pointer-events:none;}

  .ecc-topbar{ background:rgba(2,2,2,.80); backdrop-filter:blur(10px); border-bottom:1px solid rgba(255,255,255,.05); }
  .ecc-icon-btn{ width:40px; height:40px; border-radius:9999px; border:0; background:transparent; color:rgba(212,175,55,.85); display:flex; align-items:center; justify-content:center; }
  ...
  .ecc-step-kicker{ color:rgba(212,175,55,.95); font-family:"Noto Sans",system-ui,sans-serif; font-size:11px; font-weight:800; letter-spacing:.20em; text-transform:uppercase; }
  .ecc-dot{ height:4px; width:10px; border-radius:9999px; background:rgba(212,175,55,.35); }
  .ecc-dot--done{ width:18px; background:rgba(212,175,55,.45); }
  .ecc-dot--active{ width:34px; background:var(--ecc-primary); box-shadow:0 0 8px rgba(212,175,55,.60); }
  ...
  .ecc-mini{ font-size:11px; letter-spacing:.18em; text-transform:uppercase; color:rgba(212,175,55,.85); font-weight:700; }
  .ecc-tier-title{ font-size:18px; font-weight:800; color:rgba(212,175,55,.95); margin-top:2px; }
  .ecc-amount{ font-size:28px; font-weight:900; color:rgba(212,175,55,.95); }
  .ecc-tier-img{
  ...
  .ecc-pay-title{ color:rgba(212,175,55,.95); font-weight:700; font-size:16px; padding-left:2px; }
  .ecc-method{
  ...
  .ecc-lab{ color:rgba(212,175,55,.85); font-weight:700; font-size:14px; padding-left:2px; }
  .ecc-inp{
    height:56px;
    border-radius:14px;
  ...
  .ecc-inp::placeholder{ color:rgba(212,175,55,.35); }
  .ecc-inp:focus{ border-color:var(--ecc-primary) !important; box-shadow:0 0 0 .15rem rgba(212,175,55,.15) !important; }
  .ecc-ic{ position:absolute; left:14px; top:50%; transform:translateY(-50%); color:rgba(212,175,55,.95); }
  .ecc-check{ background:rgba(10,10,10,.90) !important; border-color:rgba(212,175,55,.55) !important; }
  .ecc-check:checked{ background:var(--ecc-primary) !important; border-color:var(--ecc-primary) !important; }
  .ecc-save{ color:rgba(212,175,55,.85); font-size:14px; font-weight:600; }
  ...
  .ecc-sec{ color:rgba(212,175,55,.80); font-size:12px; opacity:.9; }
