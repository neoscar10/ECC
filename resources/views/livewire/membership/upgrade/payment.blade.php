  ...
  <main class="container-fluid px-4 pt-4 pb-4 position-relative z-2">
    <div class="mx-auto ecc-max">
      ...
      <form wire:submit.prevent="submit" class="d-flex flex-column gap-3">
        ...
        <div class="ecc-pay-info-box p-4 rounded-4 mb-3" style="background: var(--ecc-bg-surface-2); border: 1px solid var(--ecc-primary-border);">
          <div class="d-flex align-items-start gap-3">
            <span class="material-symbols-outlined text-ecc" style="font-size: 32px;">verified_user</span>
            <div>
              <h5 class="ecc-pay-subtitle mb-1" style="color: var(--ecc-primary); font-weight: 700; font-size: 16px;">Secure Payment Routing</h5>
              @if($method === 'cashfree')
                <p class="mb-0 text-muted small" style="line-height: 1.5; color: var(--ecc-text-secondary) !important;">You will be redirected to the secure Cashfree payment gateway to complete your transaction. You can pay via Credit/Debit Cards, UPI, Netbanking, or Wallets.</p>
              @else
                <p class="mb-0 text-muted small" style="line-height: 1.5; color: var(--ecc-text-secondary) !important;">You will be redirected to the secure Razorpay payment gateway to complete your transaction. You can pay via Credit/Debit Cards, UPI, Netbanking, or Wallets.</p>
              @endif
            </div>
          </div>
        </div>
...
<style>
  :root{
    --ecc-bg: var(--ecc-bg-page); --ecc-surface: var(--ecc-bg-surface); --ecc-border: var(--ecc-primary-border);
  }
  ...
  .ecc-bg-grad{ position:absolute; inset:0; background: var(--ecc-bg-grad, linear-gradient(to bottom, transparent, rgba(2,2,2,.80), rgba(2,2,2,1))); pointer-events:none;}
  ...
  html[data-theme="light"] .ecc-bg-glow { background: rgba(199, 167, 90,.03); }

  .ecc-topbar{ background: var(--ecc-bg-nav-transparent, rgba(2,2,2,.80)); backdrop-filter:blur(10px); border-bottom:1px solid var(--ecc-border-soft); }
  ...
  .ecc-icon-btn:hover{ background: var(--ecc-bg-hover); }
  ...
  .ecc-step-kicker{ color:var(--ecc-primary); font-family:"Noto Sans",system-ui,sans-serif; font-size:11px; font-weight:800; letter-spacing:.20em; text-transform:uppercase; }
  ...
  .ecc-mini{ font-size:11px; letter-spacing:.18em; text-transform:uppercase; color:var(--ecc-primary); font-weight:700; }
  .ecc-tier-title{ font-size:18px; font-weight:800; color:var(--ecc-text-primary); margin-top:2px; }
  .ecc-amount{ font-size:28px; font-weight:900; color:var(--ecc-primary); }
  .ecc-tier-img{
  ...
  .ecc-tier-img:after{ content:""; position:absolute; inset:0; background:rgba(0,0,0,.10); }

  .ecc-pay-title{ color:var(--ecc-text-primary); font-weight:700; font-size:16px; padding-left:2px; }
  .ecc-method{
  ...
  .ecc-method__opt.is-on{
    ...
    color:#111;
    box-shadow:0 6px 18px var(--ecc-primary-shadow);
  }

  .ecc-lab{ color:var(--ecc-text-primary); font-weight:700; font-size:14px; padding-left:2px; }
  .ecc-inp{
    height:56px;
    border-radius:14px;
  ...
  .ecc-inp::placeholder{ color:var(--ecc-text-subtle); }
  .ecc-inp:focus{ border-color:var(--ecc-primary) !important; box-shadow:0 0 0 .15rem rgba(199, 167, 90,.15) !important; }
  .ecc-ic{ position:absolute; left:14px; top:50%; transform:translateY(-50%); color:var(--ecc-primary); }
  .ecc-check{ background: var(--ecc-bg-input) !important; border-color:var(--ecc-primary-border) !important; }
  .ecc-check:checked{ background:var(--ecc-primary) !important; border-color:var(--ecc-primary) !important; }
  .ecc-save{ color:var(--ecc-text-secondary); font-size:14px; font-weight:600; }
  ...
  .ecc-paybtn{
    ...
    box-shadow:0 0 15px var(--ecc-primary-shadow) !important;
  }
  .ecc-paybtn:hover{ background:#eac855 !important; }
  .ecc-sec{ color: var(--ecc-text-secondary); font-size:12px; }
  .ecc-sec .material-symbols-outlined{ color: var(--ecc-primary); font-size:16px; }

  .ecc-err{ font-family:"Noto Sans",system-ui,sans-serif; color:var(--ecc-danger); font-size:12px; font-weight:600; }
